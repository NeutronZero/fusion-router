# FusionRouter v0.11.0 Release Governance Platform — System Architecture Specification & Engineering Reference

> **Classification**: Production-Grade Technical Architecture Document
> **Version**: 0.11.0 | **Edition**: Rust 2021 | **Date**: 2026-07-29
> **Repository**: `fusion-router` | **Governance**: ADR-032 through ADR-036
> **Status**: ✅ Complete — 814 tests passing, 0 failures, 0 warnings on `cargo check`.
...
3. [Subsystem Engineering Reference](#3-subsystem-engineering-reference)
   ...
   - 3.18 [Production Hardening](#318-production-hardening)
    ...
    - 3.20 [Release Governance Subsystem](#320-release-governance-subsystem)
      - 3.20.1 [Feature Gate Infrastructure](#3201-feature-gate-infrastructure)
      - 3.20.2 [Release Gate Framework](#3202-release-gate-framework)
      - 3.20.3 [Release Gates Catalog](#3203-release-gates-catalog)
      - 3.20.4 [Release Policy Engine](#3204-release-policy-engine)
      - 3.20.5 [Evidence Preservation & Attestation](#3205-evidence-preservation--attestation)
      - 3.20.6 [CLI Tooling & Governance Workflow](#3206-cli-tooling--governance-workflow)
...
graph TD
    ...
    subgraph "Release Governance (Epic M)"
        FG["FeatureGate<br/>FeatureRegistry"]
        GR["GateRunner<br/>8 Release Gates"]
        PE["PolicyEngine<br/>policy.yaml"]
        AT["AttestationBuilder<br/>Signer + Archive"]
        CLI_G["CLI Binary<br/>fusion.exe"]
    end
    ...
    style FG fill:#922b21,color:#fff
    style GR fill:#922b21,color:#fff
    style PE fill:#922b21,color:#fff
    style AT fill:#922b21,color:#fff
    style CLI_G fill:#922b21,color:#fff
```
...
### 3.20 Release Governance Subsystem

The Release Governance subsystem (Epic M) provides **deterministic, policy-driven release assurance** through 8 release gates, a policy evaluation engine, and cryptographic attestation preservation. It is designed as a standalone subsystem that operates independently from the request pipeline — gates can be checked, releases can be evaluated, and attestations can be generated without any runtime dependency.

#### 3.20.1 Feature Gate Infrastructure
...
pub enum FeatureState { Enabled, Disabled }
```

The `FeatureRegistry` supports **derived feature lookup**: `is_effectively_enabled()` checks both runtime state and compile-time feature gates (`#[cfg(feature = ...)]`). Configuration overrides are applied via `FeatureGateSubscriber` implementing the `ConfigSubscriber` trait, enabling live toggling through config reload.

**Registered features** (v0.11.0):

| Flag | Stability | Default | Description |
|------|-----------|---------|-------------|
| `Streaming` | `Stable` | Enabled | Streaming response support |
| `Replay` | `Experimental` | Disabled | Request replay execution |
| `ConnectorHealth` | `Stable` | Enabled | Connector health checking |
| `SemanticCache` | `Experimental` | Disabled | Semantic vector caching |
| `WasmPlugins` | `Deprecated` | Disabled | WASM plugin sandbox |

#### 3.20.2 Release Gate Framework

```rust
// src/release/gate.rs
pub enum GateId { Sdk1, Replay1, Upgrade1, Determinism1, Plugin1, Strategy1, Provider1, Connector1 }
pub enum GateCategory { Compatibility, Replay, Upgrade, Determinism, Certification }
...
The `GateRunner` implements FIFO execution order, supporting `run_all()` (batch) and `run_one(id)` (single gate). Returned `GateExecution` distinguishes **compatibility failures** (`Success { passed: false }`) from **execution errors** (`ExecutionError`) — both set the overall report to `FAIL`.
...
#### 3.20.3 Release Gates Catalog

| GateId | Name | Category | Required | Description |
|--------|------|----------|----------|-------------|
| **SDK-1** | SemVer Compatibility | Compatibility | Yes | Verifies `cargo semver-checks` against the plugin API crate |
| **RPL-1** | Replay Compatibility | Replay | Yes | Validates snapshot deserialization, schema & format version compatibility |
| **UPG-1** | Configuration Upgrade | Upgrade | No | Ensures historical config files parse correctly through the current `AppConfig` parser |
| **DET-1** | Planner Determinism | Determinism | No | Two-compilation hash comparison (returns `ToolNotAvailable` until full compiler pipeline exists) |
| **PLG-1** | Plugin Conformance | Certification | No | Plugin manifest, SDK version compatibility, symbol export validation |
| **STR-1** | Strategy Conformance | Certification | No | Strategy registration, compilation pass compatibility |
| **PRV-1** | Provider Conformance | Certification | No | Provider catalog, pricing schema, model identifier validation |
| **CON-1** | Connector Conformance | Certification | No | Connector protocol schema, serialization compatibility |

Each gate follows a three-layer architecture:

1. **Backend trait** — abstracts data source (filesystem, mock, future CI API)
2. **Filesystem backend** — reads from `tests/fixtures/manifest.yaml` via shared `FixtureLoader`
3. **Mock backend** — `#[cfg(test)]` implementation for deterministic unit testing

**Fixture infrastructure** (`src/release/fixture.rs`, `src/release/fixture_loader.rs`):
...
impl FixtureLoader {
    pub fn load_manifest(&self) -> Result<FixtureManifest, GateError>;
    pub fn find_files(&self, dir: &Path, ext: &str) -> Result<Vec<PathBuf>, GateError>;
}
```

The `FixtureLoader` centralizes manifest loading and file traversal, eliminating duplicate parsing between gates. `discover_fixtures()` preserves **manifest declaration order** — only sorting when no manifest exists (future directory-scanning mode).

#### 3.20.4 Release Policy Engine

```rust
// src/release/policy.rs
pub enum ReleaseEnvironment { Development, Staging, Production }
...
pub enum ReleaseDecision { Approved, Blocked, AdvisoryOnly }
```

The `PolicyEvaluator` evaluates gate results against per-environment policy:

- **Staging**: Advisory-only gates are ignored; required gate failures block the release
- **Production**: All gates enforced; required failures block; advisory failures require human override
- **Development**: Auto-approved if no required gate fails with a compatibility error (execution errors ignored)

**Waiver support** (`src/release/waiver.rs`): Waivers are loaded from `waivers.yaml` and matched by `(gate_id, environment, reason)`. An active waiver converts a required-gate failure into an advisory-only notification.

#### 3.20.5 Evidence Preservation & Attestation

The attestation subsystem provides **cryptographically signed, tamper-evident** release records.
...
pub struct ReleaseAssessment {
    ...
    pub version: String,
    pub environment: ReleaseEnvironment,
    pub gates_evaluated: Vec<GateResult>,
    pub decision: ReleaseDecision,
    pub waived_failures: Vec<Waiver>,
    pub timestamp: Instant,
}

pub struct AttestationEnvelope {
    pub payload: ReleaseAssessment,
...
**4-phase verification** (`AttestationVerifier`):

| Phase | Check | Failure Consequence |
|-------|-------|---------------------|
| 1. Schema | JSON parse & `AttestationEnvelope` field validation | Reject malformed attestations |
| 2. Canonical | Re-compute canonical JSON hash against stored hash | Detect payload tampering |
| 3. Signature | Verify Ed25519 signature over canonical hash | Detect key compromise or signature forgery |
| 4. Semantic | Validate assessment semantic consistency (version match, required gates present) | Detect logical corruption |

The `FilesystemArchiveBackend` stores attestations as append-only JSON files in `.fusion/attestations/`. Each file is named with a deterministic assessment ID (`asm-<hash>.json`). The `Signer` trait supports mock and Ed25519 implementations.

```rust
// build_default_runner() — all gates registered via FixtureLoader
pub fn build_default_runner(workspace_root: PathBuf, baseline_ref: &str) -> GateRunner {
    // Registers SDK-1, RPL-1, UPG-1, DET-1, PLG-1, STR-1, PRV-1, CON-1
}
```

#### 3.20.6 CLI Tooling & Governance Workflow

The `fusion` CLI binary (`src/bin/fusion.rs`) provides 6 governance commands:

| Command | Purpose |
|---------|---------|
| `fusion gates list` | List all registered release gates with IDs, names, descriptions |
| `fusion gates check` | Execute all gates and display a formatted GateReport |
| `fusion gates explain <ID>` | Show gate metadata (category, required, introduced version) |
| `fusion gates evaluate --env <ENV>` | Evaluate gate results against environment policy, producing a ReleaseDecision |
| `fusion gates attest --env <ENV>` | Run evaluate, build ReleaseAssessment, sign, and archive attestation |
| `fusion gates verify-attestation <path>` | 4-phase verification of a signed attestation file |

The CLI is a thin renderer — all governance logic lives in `src/release/` and `src/feature_gate/`. Registration is owned entirely by `build_default_runner()`; the CLI never references gates by name.
...
├── src/
│   ├── bin/
│   │   └── fusion.rs                  # CLI binary: gates list/check/explain/evaluate/attest/verify-attestation, trace timeline/events, features list
│   ├── main.rs                        # Entry point: Tokio runtime, Axum router, provider/middleware setup
│   ├── lib.rs                         # Public module re-exports (30+ modules + conditional wasm + release governance)
...
│   │
│   ├── release/
│   │   ├── mod.rs                     # Release governance module declarations
│   │   ├── gate.rs                    # GateId, GateCategory, GateContext, GateResult, GateCheck, GateMetadata, GateError, GateExecution, ReleaseGate trait
│   │   ├── runner.rs                  # GateRunner: FIFO execution run_all/run_one
│   │   ├── report.rs                  # GateReport: JSON/text output with overall pass/fail
│   │   ├── fixture.rs                 # FixtureManifest, FixtureEntry, FixtureKind, ManifestEntry, ExpectedOutcome types
│   │   ├── fixture_loader.rs          # FixtureLoader: shared manifest loading, file traversal, fixture discovery
│   │   ├── policy.rs                  # ReleasePolicy, ReleaseEnvironment, ReleaseDecision
│   │   ├── evaluator.rs               # PolicyEvaluator: gate results vs per-environment policy
│   │   ├── assessment.rs              # ReleaseAssessment: structured release evaluation record
│   │   ├── waiver.rs                  # WaiverEngine: waiver matching from waivers.yaml
│   │   ├── signing.rs                 # Signer trait, Ed25519Signer, MockSigner
│   │   ├── attestation.rs             # AttestationBuilder, AttestationEnvelope, canonical serialization
│   │   ├── verifier.rs                # AttestationVerifier: 4-phase verification pipeline
│   │   ├── archive.rs                 # FilesystemArchiveBackend: append-only .fusion/attestations/
│   │   ├── bootstrap.rs               # build_default_runner() — single assembly point for all gates
│   │   └── gates/
│   │       ├── mod.rs                 # Gate module declarations
│   │       ├── semver.rs              # SemVerGate (SDK-1): SemVer compatibility via cargo semver-checks
│   │       ├── replay.rs              # ReplayGate (RPL-1): snapshot deserialization + schema/format compatibility
│   │       ├── upgrade.rs             # UpgradeGate (UPG-1): historical config load compatibility
│   │       ├── determinism.rs         # DeterminismGate (DET-1): graph hash determinism check (scaffold)
│   │       ├── plugin.rs              # PluginGate (PLG-1): plugin manifest, SDK version, symbol export check
│   │       ├── strategy.rs            # StrategyGate (STR-1): strategy registration + compilation pass check
│   │       ├── provider.rs            # ProviderGate (PRV-1): provider catalog, pricing, model ID check
│   │       └── connector.rs           # ConnectorGate (CON-1): connector protocol schema + serialization check
│   │
│   ├── feature_gate/
│   │   ├── mod.rs                     # FeatureFlag, FeatureDefinition, FeatureRegistry, FeatureState, Stability
│   │   └── config_subscriber.rs       # FeatureGateSubscriber: ConfigSubscriber for live feature toggling
│   │
│   ├── policy/
...
│   ├── common/
│   │   └── mod.rs                     # Test helpers: re-exports FixtureLoader, test_loader()
│   ├── fixtures/
│   │   └── manifest.yaml              # Fixture manifest: configs, snapshots, plugins, strategies, providers, connectors
...
│   ├── release_gate_tests.rs          # Release gate integration tests (6 tests)
│   ├── release_attestation_tests.rs   # Attestation end-to-end tests (2 tests)
│   ├── release_policy_tests.rs        # Policy evaluation tests (2 tests)
...
| `prometheus-metrics` | ✅ Yes | `src/telemetry/metrics.rs`, telemetry module | `prometheus = "0.13"` |
...
| Backlog Feature | Resolution |
|----------------|------------|
| **Streaming Resource Metering** | `StreamMeter` with token/cost counting; `MeteredStream` wrapping SSE streams; `StreamMeterReport` on stream completion |
| **Hot-Reload Configuration** | `ConfigManager` with two-phase prepare/commit/rollback; `ConfigSubscriber` trait for live updates; `ConfigError` types |
| **Release Gate Foundation** | `FeatureRegistry` + `FeatureGateSubscriber`; `ReleaseGate` trait + `GateRunner` + `GateReport`; `SemVerGate` (SDK-1) via `build_default_runner()` |
| **Release Gate Ecosystem** | 7 additional gates: RPL-1, UPG-1, DET-1, PLG-1, STR-1, PRV-1, CON-1; shared `FixtureLoader` infrastructure |
| **Release Policy Engine** | `ReleasePolicy`, `PolicyEvaluator`, per-environment gate requirements; `WaiverEngine` with `waivers.yaml` support |
| **Release Attestation** | `AttestationBuilder` (canonical serialization); `Ed25519Signer`; `AttestationEnvelope`; `FilesystemArchiveBackend`; `AttestationVerifier` (4-phase) |
| **Governance CLI** | `fusion gates list/check/explain/evaluate/attest/verify-attestation`; `fusion features list` |
| **Fixture Infrastructure** | `FixtureManifest` / `FixtureLoader` in production code; manifest-order preservation; `discover_fixtures()` shared by all backends |
...
| **Behavioral Replay Gates** | `src/release/gates/` | Regression-aware replay that validates execution output correctness, not just structural compatibility |
| **CI Integration** | `GateRunner` CI adapter | Headless gate execution in GitHub Actions / GitLab CI pipelines with JUnit XML output |
| **Admin API** | `GET /admin/releases` | REST endpoints for release assessment history, attestation query, and policy management |

---

> **Document Revision 9 (v0.11.0 Release)**: Comprehensive architecture document covering all v0.10.0 + v0.11.0 subsystems — streaming resource metering, live configuration reload (ADR-032), release governance foundation (Epic M / ADR-033 through ADR-036). 814 tests passing, 0 failures.
...
| `docs/roadmap-v0.11.md` | v0.11 roadmap status (✅ Complete) |
| `docs/superpowers/specs/2026-07-27-release-gate-foundation.md` | Sprint M1 — Release Gate Foundation design spec |
| `docs/superpowers/specs/2026-07-27-compatibility-upgrade-assurance.md` | Sprint M2 — Compatibility & Upgrade Assurance design spec |
| `docs/superpowers/plans/2026-07-27-release-gate-foundation.md` | Sprint M1 implementation plan (8 tasks) |
| `docs/superpowers/plans/2026-07-27-compatibility-upgrade-assurance.md` | Sprint M2 implementation plan (6 tasks) |
| `docs/release/release_gate_spec.md` | Release gate specification: architecture, gate catalog, waiver process, archive layout |
| `docs/fusionrouter_architecture_v0.11.0.md` | This document — v0.11.0 architecture specification |
| `CHANGELOG.md` | Release changelog with per-sprint entries |
