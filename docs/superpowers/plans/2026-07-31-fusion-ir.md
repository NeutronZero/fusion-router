...
- **Canonical ordering:** nodes sorted by `id`; edges sorted by `(from, to, kind)`; `config` sorted via `BTreeMap`. Determinism is a property of the IR, not of builder call order.
