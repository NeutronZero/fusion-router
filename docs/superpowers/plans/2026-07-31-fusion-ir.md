...
- **Serde contract:** the graph structs (`WorkflowIR`, `WorkflowNode`, `WorkflowEdge`) derive `Debug, Clone, Serialize, Deserialize` with `#[serde(deny_unknown_fields)]`; enums derive `Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize`; `WorkflowMetadata` (frozen verbatim) omits `deny_unknown_fields` deliberately so future metadata fields don't break older parsers.
