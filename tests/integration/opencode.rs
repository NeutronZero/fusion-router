...
impl ChatProvider for StreamingMockProvider {
    ...
    async fn chat_stream(
        ...
        futures::stream::BoxStream<'static, anyhow::Result<fusion_router::types::ChatStreamChunk>>,
    > {
...
async fn test_native_streaming_end_to_end() {
    ...
    let registry = Arc::new(fusion_router::providers::registry::ProviderRegistry::new(
        target,
    ));
    ...
    assert!(
        data_events >= 5,
        "expected >=5 SSE events (3 chunks + usage + DONE), got {data_events}"
    );
...
async fn test_chat_completion_endpoint() {
    ...
    let evidence: Arc<dyn EvidenceRepository + Send + Sync> = Arc::new(NoopEvidence);
    let config = AppConfig::load("config/default.yaml").unwrap_or_else(|_| AppConfig {
        unsafe_dev: false,
...
async fn test_runtime_deny_policy_blocks_chat_completions() {
    ...
    assert_eq!(
        resp.status(),
        200,
        "request must succeed before any policy exists"
    );
    ...
    assert_eq!(
        resp.status(),
        200,
        "service must recover after policy removal"
    );
}
...
async fn test_dag_split_join_workflow() {
    ...
    assert!(
        exec_result.success,
        "DAG workflow should complete successfully"
    );

    let succeeded: Vec<_> = instance
        .node_states
        .values()
    ...
    assert_eq!(
        succeeded.len(),
        5,
        "All 5 nodes should succeed (Split + A + B + Join + Final)"
    );
}

use fusion_router::config::{
    AuthConfig, CorsConfig, LoggingConfig, RateLimitingConfig, ResourceConfig, ServerConfig,
    StrategyConfig, ToolsConfig,
};
...
fn test_config() -> AppConfig {
    AppConfig {
        ...
        auth: AuthConfig {
            enabled: true,
            api_keys: vec!["test-key:operator".into(), "chat-key".into()],
        },
...
async fn test_middleware_stack_rejects_unauthenticated() {
    ...
    let app = Router::new()
        ...
        .route(
            "/health",
            get(fusion_router::server::health::health_handler),
        )
        .route(
            "/metrics",
            get(fusion_router::server::handlers::metrics_handler),
        )
        .layer(TraceLayer::new_for_http())
        ...
        .layer(axum::Extension(
            fusion_router::middleware::auth::AuthHandle::from_config(&config.auth),
        ))
        .layer(fusion_router::middleware::cors::cors_layer_from_config(
            &config.server.cors,
        ))
        .with_state(state);
...
async fn test_executions_and_operations_auth_enforcement() {
    ...
    let ops_policy_admin = Arc::new(PolicyAdmin::new(
