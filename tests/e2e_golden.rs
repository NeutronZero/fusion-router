...
use fusion_compiler::{policy, CompilerEngine};
...
async fn test_e2e_golden_workflow() {
    ...
    let planning_ir = planner
        .plan_with_intent("Build a web app", ExecutionIntent::Balanced)
        .expect("Planner must produce WorkflowIR");

    let node_count = planning_ir.nodes().len();
    assert_eq!(
        node_count, 3,
        "Balanced intent must produce 3 nodes (gen → gen → judge), got {node_count}"
    );

    // ── Step 2: Adapter converts planning IR → execution IR ────────────────
    let exec_ir =
        workflow_to_types(&planning_ir).expect("Adapter must convert planning IR to execution IR");
    ...
    let (report, graph) = compiler
        .compile_and_lower("Build a web app", &exec_ir)
        .await
    ...
    let provider: Arc<dyn fusion_runtime::ChatProvider> =
        Arc::new(MockProvider::default_response());
...
async fn test_e2e_golden_speed_workflow() {
    ...
    assert_eq!(
        planning_ir.nodes().len(),
        1,
        "Speed intent must produce 1 node"
    );

    let exec_ir = workflow_to_types(&planning_ir).expect("Adapter");
    let compiler = CompilerEngine::new();
    let (report, graph) = compiler
        .compile_and_lower("Quick fix", &exec_ir)
        .await
        .expect("Compiler");
...
async fn test_e2e_golden_quality_workflow() {
    ...
    let planner = PlannerService::new(capability_system);
    let planning_ir = planner
        .plan_with_intent("Build a web app", ExecutionIntent::Quality)
        .expect("Planner must produce WorkflowIR");

    assert_eq!(
        planning_ir.nodes().len(),
        5,
        "Quality intent must produce 5 nodes"
    );
...
async fn test_e2e_dead_node_elimination() {
    ...
    let planner = PlannerService::new(capability_system);
    let planning_ir = planner
        .plan_with_intent("Quick fix", ExecutionIntent::Balanced)
        .expect("Planner");
    ...
    assert_eq!(
        exec_ir.nodes.len(),
        4,
        "IR must have 4 nodes (3 live + 1 orphan)"
    );

    let compiler = CompilerEngine::new();
    let (_report, graph) = compiler
        .compile_and_lower("dead node test", &exec_ir)
        .await
        .expect("Compiler");

    assert_eq!(graph.nodes.len(), 3, "orphan must be eliminated from graph");
    assert!(
        !graph.nodes.iter().any(|n| n.id == orphan_id),
...
async fn test_e2e_policy_deny_blocks_compilation() {
    ...
    let rm: Arc<dyn fusion_kernel::resource::ResourceManager> = Arc::new(
        fusion_kernel::resource::StubResourceManager::new(fusion_kernel::resource::Quota {
            max_daily_cost: fusion_core::NanoUSD::from_nanos(u64::MAX),
            max_daily_tokens: u64::MAX,
        }),
    );
    let engine =
        fusion_compiler::build_compiler(fusion_types::ModelCatalog::default(), rm, Some(policy_ir));
