...
use fusion_router::types::{ExecutionNode, ExecutionNodeKind, RetryPolicy, StrategyKind};
...
fn test_chain_strategy_produces_pipeline() {
    ...
    assert!(
        !eg.edges.is_empty(),
        "Chain should produce at least 1 edge connecting stages"
    );
    ...
    assert!(
        gen_count >= 1,
        "Chain should include at least 1 Generate node"
    );
    assert!(
        review_count >= 1,
        "Chain should include at least 1 Review node"
    );
}
...
fn test_react_strategy_produces_loop() {
    ...
    assert_eq!(
        eg.nodes.len(),
        1,
        "ReAct should produce exactly 1 node (FeedbackLoop)"
    );
...
fn test_debate_strategy_produces_parallel_judge() {
    ...
    assert!(
        eg.nodes.len() >= 3,
        "Debate should produce at least 3 nodes (2 debaters + 1 reducer)"
    );
}
...
fn test_chain_strategy_single_stage_passthrough() {
    ...
    assert_eq!(
        eg.nodes.len(),
        1,
        "Single-stage chain should produce exactly 1 node"
    );
}
...
fn test_react_strategy_custom_max_iterations() {
    ...
    let ctx = CompilationContext::new();
    let pg = strategy
        .lower(&StrategyIR::ReAct { max_iterations: 5 }, &ctx)
        .unwrap();

    let loop_node = &pg.nodes[0];
    assert!(matches!(
        loop_node.kind,
        fusion_router::compiler::ir::PrimitiveNodeKind::FeedbackLoop { max_iterations: 5 }
    ));
}
