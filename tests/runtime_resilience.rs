...
fn test_resilience_lease_renewal_and_revoked_failover_epoch_monotonicity() {
    ...
    let lease = lease_manager
        .grant_lease("exec_901", "node_ttl", "worker_a", 50_000)
        .expect("grant");
    assert_eq!(lease.epoch, 1);
    assert!(
        lease_manager.renew_lease(&lease.lease_key),
        "live lease must renew"
    );
    assert!(!lease.is_expired(lease.granted_at_ms));
    assert!(
        lease.is_expired(lease.granted_at_ms + 50_000),
        "TTL elapsed => expired"
    );

    // Epoch must be monotonic per (exec,node) across crash/failover.
    assert!(
        lease_manager.revoke_lease(&lease.lease_key),
        "crash => revoke"
    );
    let again = lease_manager
        .grant_lease("exec_901", "node_ttl", "worker_b", 50_000)
        .expect("revoked lease allows new owner");
    assert!(
        again.epoch > lease.epoch,
        "epoch must increase across owners"
    );
}
